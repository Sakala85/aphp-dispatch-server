...
const getUser = () => {
  const userSorted0 = users.filter((user) => user.username !== "dispatch");
  const userSorted1 = userSorted0.filter((user) => user.online === 1);
  const userSorted2 = userSorted1.concat(userSorted0.filter((user) => user.online === 2));
  const userSorted = userSorted2.concat(userSorted0.filter((user) => user.online === 0));
  return userSorted;
};
